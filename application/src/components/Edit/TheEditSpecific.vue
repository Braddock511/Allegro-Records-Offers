  ...
  <span>
    <section v-if="!discogsData && !loading">
      <form class="flex flex-col items-center gap-2">
        <div class="text-2xl text-white font-bold">{{ $t("editSpecific.offerId") }}</div>
        <div class=""><input type="text" name="offer-id" v-model="offerId" required class="text-center bg-light-black rounded-lg px-10"/></div>
        <div class="flex gap-2 flex-wrap justify-center flex-column">
          <label class="label cursor-pointer gap-1">
            <span class="label-text text-base"> {{ $t("editSpecific.editPrice") }}</span>
            <input type="checkbox" checked="checked" v-model="editPrice" class="checkbox h-5 w-5 checkbox-primary bg-gray-600"/>
          </label>
          <label class="label cursor-pointer gap-1">
            <span class="label-text text-base"> {{ $t("editSpecific.editDescription") }}</span>
        ...
        <button class="btn btn-primary w-40" type="submit" @click="getOffer">{{ $t("editSpecific.edit") }}</button>
      </form>
    </section>
  </span>
  <div v-if="!loading">
    <div class="flex h-full flex-col items-center mt-10" v-if="discogsData">
      ...
      <div class="flex gap-4">
        <button class="btn btn-primary w-32" type="type" @click="editImage">{{ $t("editSpecific.clearImage") }}</button>
        <button class="btn btn-primary w-32" type="type" @click="discogsData = ''; alert = {}; ">{{ $t("table.back") }}</button>
      </div>
      <div id="tableData" className="overflow-x-auto w-full custom-scrollbar mt-20">
        <table class="mt-10 table table-sm">
          <thead class="thead-css h-14 text-base">
            <tr>
              ...
              <th v-if="editPrice">
                <div class="relative">
                  <label for="condition" class="absolute flex justify-center items-center -top-[2px] left-1 h-[1.5px] bg-oceans px-1 text-xs font-semibold" >{{ $t("table.price") }}</label >
                  <select id="condition" v-model="condition" @change="this.price = ''" class="text-gray-100 select-css font-semibold h-8 p-1 rounded-md placeholder:text-center border-1 bg-oceans px- outline-none" >
              ...
              <th>{{ $t("editSpecific.listingSimilar") }}</th>
            </tr>
          </thead>
          <tbody class="tab-css">
            <tr class="bg-darker-gray">
              <th class="text-transparent"></th>
              <td><input type="text" name="label" class="h-8 rounded-md placeholder:text-center bg-oceans px-1 font-semibold hover:bg-opacity-0" placeholder="Label" v-model="label" /></td>
              <td><input type="text" name="country" class="h-8 rounded-md placeholder:text-center bg-oceans px-1 font-semibold hover:bg-opacity-0" placeholder="Country" v-model="country" /></td>
              <td><input type="text" name="year" class="h-8 rounded-md placeholder:text-center bg-oceans px-1 font-semibold hover:bg-opacity-0" placeholder="Year" v-model="year" /></td>
              <td v-if="editPrice"><input type="text" name="price" class="text-center text-gray-100 h-8 rounded-md placeholder:text-center bg-oceans px-1 font-semibold hover:bg-opacity-0" placeholder="Price" v-model="price" /></td>
              <td><button class="btn btn-primary w-full" type="type" style="   max-height: 2rem !important;   min-height: 2rem !important; " @click="editOffer" >{{ $t("editSpecific.edit") }}</button></td>
            ...
            <tr v-for="(data, index) in discogsData.output[0].information" v-if="discogsData && discogsData.output" class="mapping odd:bg-dark-gray even:bg-lighter-gray">
              ...
              <td v-if="editPrice && data.price[condition == 'Excellent (EX)' ? 'Very Good Plus (VG+)' : condition] !== undefined">
                <div class="relative">
                  <label :for="`price-${index}`" :class="   index % 2 === 0 ? 'bg-lighter-gray  ' : 'bg-dark-gray' " class="absolute flex justify-center items-center h-1 text-gray-300 left-1 bg-oceans px-1 text-sm font-semibold">
                    Want: {{ data.community.want }} | Have: {{ data.community.have }}</label>
                  <input type="number" name="price" :id="`price-${index}`" class="text-center text-gray-100 h-8 rounded-md placeholder:text-center bg-oceans px-1 font-semibold" min="1" 
                  :placeholder="
                      roundedPrice(
                        data.price[
              ...
              <td v-if="   editPrice &&   data.price[     condition == 'Excellent (EX)'       ? 'Very Good Plus (VG+)'       : condition   ] === undefined ">
                <div class="relative">
                  ...
                  <input type="number" name="price" min="1" v-model="price" :id="`price-${index}`" class="h-8 rounded-md placeholder:text-center bg-oceans px-1 font-semibold" />
                </div>
              </td>
              <td><button class="btn btn-primary w-full" style="   max-height: 1.75rem !important;   min-height: 1.75rem !important; " type="type" @click="editOffer(data)">{{ $t("editSpecific.edit") }}</button></td>
              <td><button class="btn btn-primary w-full" style="   max-height: 1.75rem !important;   min-height: 1.75rem !important; " type="type" @click="   listing_similar = true;   editOffer(data); ">{{ $t("table.send") }}</button></td>
            </tr>
          </tbody>
        </table>
      </div>
...
export default {
  ...
  methods: {
    async getOffer() {
      this.loading = true;
      this.allegroData = (await axios.post(`${baseUrl}/allegro-offer`, {userKey: this.userKey, offerId: this.offerId,})).data;
      if (this.allegroData.error || this.allegroData.errors) {
        ...
        await axios.post(`${baseUrl}/discogs-information`, {
    ...
    async editImage() {
      ...
      axios.post(`${baseUrl}/clear-image`, {
          userKey: this.userKey,
          image: this.img[0],
        }).then((response) => {
      ...
      axios.post(`${baseUrl}/allegro-edit-image`, {
          userKey: this.userKey,
          offerId: this.offerId,
          images: newImages,
        }).then((response) => {
    ...
    async search() {
      this.loading = true;
      axios.post(
          `${baseUrl}/discogs-information-new-search`,
          {
            userKey: this.userKey,
